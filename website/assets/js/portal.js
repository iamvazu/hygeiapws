/**
 * HYGEIA ENTERPRISE — PRODUCTION CLIENT PORTAL ENGINE (v3.0.0)
...
(function() {
  ...
  const BUSINESS_UNIT = document.body.dataset.businessUnit || 'pws';
  ...
  if (formRegister) {
    formRegister.addEventListener('submit', async (e) => {
      ...
      const payload = {
        ...
        active_plan: BUSINESS_UNIT === 'janitorial' 
          ? 'Silicon Valley Commercial Facility Program' 
          : 'Commercial Bin & Pressure Wash Maintenance'
      };
  ...
  function renderClientDashboard(data) {
    ...
    if (elCompany) elCompany.textContent = client.company_name || 'Commercial Client';
    ...
    if (elNextService) elNextService.textContent = client.next_service_date || 'Pending Schedule';
    if (elActivePlan) elActivePlan.textContent = client.active_plan;
    if (elMonthlyRate) elMonthlyRate.textContent = client.monthly_rate > 0 ? `$${client.monthly_rate.toLocaleString('en-US')}/mo` : 'Custom Billing';
      ...
      tbody.innerHTML = `
        <tr>
          <td colspan="6" style="text-align:center;padding:32px;color:var(--text-muted);">
            ...
            No extra work orders requested yet. Use the button above to request special service.
          </td>
        </tr>
      `;
      return;
  ...
  if (formWorkOrder) {
    formWorkOrder.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        alert('✅ Work Order Submitted Successfully! Hygeia Dispatch operations has received your request.');
